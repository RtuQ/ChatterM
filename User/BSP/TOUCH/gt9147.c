...
u8 GT9147_Init(void)
{
    ...
    RCC_AHB1PeriphClockCmd(RCC_AHB1Periph_GPIOH,ENABLE);			//开启GPIOH时钟
    RCC_AHB1PeriphClockCmd(RCC_AHB1Periph_GPIOI,ENABLE);						//开启GPIOI时钟
                
    //PH7
    GPIO_InitStructure.GPIO_Pin=GPIO_Pin_7;            //PH7
    ...
    //PI8
    GPIO_InitStructure.GPIO_Pin=GPIO_Pin_8;            //PI8
    ...
    GPIO_InitStructure.GPIO_Pin=GPIO_Pin_7;            //PH7
    GPIO_InitStructure.GPIO_Mode=GPIO_Mode_IN;      //输入
    GPIO_InitStructure.GPIO_PuPd=GPIO_PuPd_NOPULL;          //不带上下拉，浮空输入
    GPIO_InitStructure.GPIO_Speed=GPIO_Speed_50MHz;     //高速
    GPIO_Init(GPIOH,&GPIO_InitStructure);     //初始化
	bsp_DelayMS(100);  
...
u8 GT9147_Scan(u8 mode)
{
	...
	if((t%10)==0||t<10)//空闲时,每进入10次CTP_Scan函数才检测1次,从而节省CPU使用率
	{
		...
		if((mode&0XF)&&((mode&0XF)<6))
		{
			...
			res=1;
			if(tp_dev.x[0]>LCD_PIXEL_WIDTH||tp_dev.y[0]>LCD_PIXEL_HEIGHT)//非法数据(坐标超出了)
			{ 
