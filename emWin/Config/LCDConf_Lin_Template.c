...
*                                       6. 注释掉所有休眠指令，暂时用不上。
*
*		V1.1    2016-01-28    Eric2013  1. 取消掉变量定义uint16_t Width, Height, HSW, VSW, HBP, HFP, VBP, VFP
*                                          前面的__IO类型。
（
...
*/
#define LCD_LAYER0_FRAME_BUFFER  SDRAM_BANK_ADDR
...
/* 8. 配置图层2的的颜色模式和分辨率大小 */
#define COLOR_MODE_1      _CM_ARGB1555      //若配置为ARGB888会导致在使用储存设备时屏幕出现黑色区域
...
static void _LCD_InitController(int LayerIndex) 
{
	...
	LTDC_Layer_InitStruct.LTDC_HorizontalStart = HSW + HBP + 1;
	LTDC_Layer_InitStruct.LTDC_HorizontalStop = (Width + LTDC_Layer_InitStruct.LTDC_HorizontalStart - 1);
	LTDC_Layer_InitStruct.LTDC_VerticalStart = VSW + VBP + 1; 
...
void LCD_X_Config(void) 
{
	...
	LCD_SetVSizeEx(0, LCD_PIXEL_WIDTH, LCD_PIXEL_HEIGHT * NUM_VSCREENS);
	#if (GUI_NUM_LAYERS > 1)
		//
